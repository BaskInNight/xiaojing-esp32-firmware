/*
 * detergent_service.c — 洗衣液投放服务层 (Round 4.2.1)
 *
...
static const char *TAG = "detergent_svc";
...
typedef struct { cmd_type_t type; detergent_request_t request; } detergent_cmd_t;
...
static void detergent_service_auto_init(void) { detergent_service_global_init(); }

static esp_err_t apply_pump_on(void)
{
    safety_request_t req = { .operation = SAFETY_OP_DETERGENT_PUMP, .enable = true,
        .request_id = s_rt.fsm.request_id, .required_position = DRUM_POS_0, .source = APP_SOURCE_SYSTEM };
    return safety_manager_apply(&req);
}
static esp_err_t apply_pump_off(void)
{
    safety_request_t req = { .operation = SAFETY_OP_DETERGENT_PUMP, .enable = false,
        .request_id = s_rt.fsm.request_id, .required_position = DRUM_POS_UNKNOWN, .source = APP_SOURCE_SYSTEM };
    return safety_manager_apply(&req);
}
static esp_err_t execute_fsm_action(detergent_fsm_action_t action)
{
    switch (action) {
    case DETERGENT_ACTION_PUMP_ON:  return apply_pump_on();
    case DETERGENT_ACTION_PUMP_OFF: return apply_pump_off();
...
static void build_terminal_event(const detergent_fsm_output_t *out, machine_event_t *event)
{
    memset(event, 0, sizeof(*event));
    event->type = MACHINE_EVENT_DETERGENT_DONE; event->request_id = s_rt.fsm.request_id;
    switch (out->terminal) {
    case DETERGENT_TERMINAL_COMPLETE: event->result = SERVICE_RESULT_OK; break;
    case DETERGENT_TERMINAL_CANCELED: event->result = SERVICE_RESULT_CANCELED; break;
    case DETERGENT_TERMINAL_TIMEOUT:  event->result = SERVICE_RESULT_TIMEOUT; break;
    case DETERGENT_TERMINAL_REJECTED: event->result = SERVICE_RESULT_REJECTED; break;
    case DETERGENT_TERMINAL_FAULT:    event->result = SERVICE_RESULT_FAULT; break;
...
static bool try_publish_terminal(void)
{
    ...
    if (err == ESP_OK) { s_rt.pending_term.valid = false; detergent_fsm_mark_terminal_emitted(&s_rt.fsm); return true; }
    return false;
}
...
static esp_err_t detergent_stop_hook(void *ctx) { (void)ctx; return detergent_service_emergency_stop(); }

static void process_fsm_output(detergent_fsm_output_t *out, uint32_t now_ms)
{
    int chain = 0;
    while (out->action != DETERGENT_ACTION_NONE && chain < MAX_CHAIN_ACTIONS) {
        esp_err_t err = execute_fsm_action(out->action);
        detergent_fsm_event_t res = { .type = DETERGENT_EVT_ACTION_RESULT, .now_ms = now_ms,
            .action_result_action = out->action, .action_result_ok = (err == ESP_OK) };
        detergent_fsm_tick(&s_rt.fsm, &res, out); chain++;
    }
    if (chain >= MAX_CHAIN_ACTIONS && out->action != DETERGENT_ACTION_NONE) {
        ESP_LOGE(TAG, "chain limit exceeded");
        detergent_fsm_event_t em = { .type = DETERGENT_EVT_EMERGENCY, .now_ms = now_ms };
        detergent_fsm_tick(&s_rt.fsm, &em, out);
        if (out->action != DETERGENT_ACTION_NONE) {
            esp_err_t err = execute_fsm_action(out->action);
            detergent_fsm_event_t res = { .type = DETERGENT_EVT_ACTION_RESULT, .now_ms = now_ms,
                .action_result_action = out->action, .action_result_ok = (err == ESP_OK) };
            detergent_fsm_tick(&s_rt.fsm, &res, out);
        }
    }
    ...
    if (try_publish_terminal()) {
        if (s_rt.fsm.terminal_event_emitted &&
            (s_rt.fsm.state == DETERGENT_FSM_COMPLETE || s_rt.fsm.state == DETERGENT_FSM_FAULT)) {
            s_rt.fsm.state = DETERGENT_FSM_IDLE;
...
static void detergent_task(void *arg)
{
    (void)arg;
    apply_pump_off();
    ...
    while (!atomic_load(&s_rt.stop_requested)) {
        ...
        if (s_rt.emerg_mailbox.pending) {
            ...
            if (off_result != ESP_OK) {
                s_rt.fsm.output_state = DETERGENT_OUTPUT_UNKNOWN;
                safety_manager_report_fault(FAULT_MCP_IO, FAULT_SEVERITY_LATCHED, (uint32_t)off_result);
            } else { s_rt.fsm.output_state = DETERGENT_OUTPUT_KNOWN_OFF; }
            s_rt.fsm.state = DETERGENT_FSM_COMPLETE; s_rt.fsm.terminal = DETERGENT_TERMINAL_FAULT;
            s_rt.fsm.fault_code = FAULT_INTERNAL; s_rt.fsm.fault_severity = FAULT_SEVERITY_RECOVERABLE;
            detergent_fsm_output_t t = { .emit_event = true, .terminal = DETERGENT_TERMINAL_FAULT,
        ...
        while (xQueueReceive(s_rt.cmd_queue, &cmd, 0) == pdTRUE) {
            ...
            if (cmd.type == CMD_RUN) {
                esp_err_t err = detergent_fsm_begin_request(&s_rt.fsm, &cmd.request, s_rt.cfg.max_single_ms, now_ms);
                if (err != ESP_OK) {
                    machine_event_t rej = {0}; rej.type = MACHINE_EVENT_DETERGENT_DONE;
                ...
                detergent_fsm_event_t e = { .type = DETERGENT_EVT_CANCEL, .now_ms = n, .cancel_request_id = cmd.request.request_id };
                detergent_fsm_output_t o; detergent_fsm_tick(&s_rt.fsm, &e, &o); process_fsm_output(&o, n);
            } else if (cmd.type == CMD_EMERGENCY) {
                uint32_t n = (uint32_t)(xTaskGetTickCount() * portTICK_PERIOD_MS);
                detergent_fsm_event_t e = { .type = DETERGENT_EVT_EMERGENCY, .now_ms = n };
                detergent_fsm_output_t o; detergent_fsm_tick(&s_rt.fsm, &e, &o); process_fsm_output(&o, n);
            }
        }
        ...
        uint32_t now_ms = (uint32_t)(xTaskGetTickCount() * portTICK_PERIOD_MS);
        if (s_rt.fsm.state == DETERGENT_FSM_RUNNING) {
            bool aa = false; safety_reject_reason_t ar = SAFETY_REJECT_NONE;
            if (safety_manager_validate_active(SAFETY_OP_DETERGENT_PUMP, DRUM_POS_0, &aa, &ar) != ESP_OK || !aa) {
                detergent_fsm_event_t em = { .type = DETERGENT_EVT_EMERGENCY, .now_ms = now_ms };
                detergent_fsm_output_t eo; detergent_fsm_tick(&s_rt.fsm, &em, &eo); process_fsm_output(&eo, now_ms);
                vTaskDelayUntil(&last_wake, pdMS_TO_TICKS(TICK_INTERVAL_MS)); continue;
            }
        }
        if (!s_rt.pending_term.valid) {
            detergent_fsm_event_t te = { .type = DETERGENT_EVT_TICK, .now_ms = now_ms,
                .position = pos, .position_valid = pv, .position_stable = ps, .position_fresh = pf, .position_motor_moving = mm };
            detergent_fsm_output_t o; detergent_fsm_tick(&s_rt.fsm, &te, &o); process_fsm_output(&o, now_ms);
        }
    ...
    /* Stop drain */
    {
        ...
        while (s_rt.fsm.state != DETERGENT_FSM_IDLE && stop_drain_retries < 8) {
            if (s_rt.pending_term.valid) {
                try_publish_terminal();
                if (s_rt.fsm.terminal_event_emitted &&
                    (s_rt.fsm.state == DETERGENT_FSM_COMPLETE || s_rt.fsm.state == DETERGENT_FSM_FAULT)) {
                ...
                if (s_rt.fsm.state != DETERGENT_FSM_IDLE) {
            ...
            detergent_fsm_event_t em = { .type = DETERGENT_EVT_EMERGENCY, .now_ms = (uint32_t)(xTaskGetTickCount() * portTICK_PERIOD_MS) };
            detergent_fsm_output_t eo; detergent_fsm_tick(&s_rt.fsm, &em, &eo); process_fsm_output(&eo, em.now_ms);
            vTaskDelay(pdMS_TO_TICKS(50));
            stop_drain_retries++;
        }
    }
    esp_err_t off_err = apply_pump_off();
...
esp_err_t detergent_service_init(const detergent_params_t *config, const xiaojing_hal_t *hal, machine_event_sink_t event_sink)
{
    ...
    detergent_fsm_init(&s_rt.fsm, config);
    s_rt.snapshot_mtx = xSemaphoreCreateMutex();
    s_rt.cmd_queue = xQueueCreate(CMD_QUEUE_DEPTH, sizeof(detergent_cmd_t));
...
esp_err_t detergent_service_start(void)
{
    ...
    esp_err_t hook_err = safety_manager_register_stop_hook(detergent_stop_hook, NULL, "detergent");
    if (hook_err != ESP_OK) { s_rt.lifecycle = LC_INITIALIZED; xSemaphoreGive(s_lc_mtx); return hook_err; }
    s_rt.hook_registered = true;
    if (xTaskCreatePinnedToCore(detergent_task, "det_svc", TASK_STACK_SIZE, NULL, TASK_PRIORITY, NULL, TASK_CORE) != pdPASS) {
        safety_manager_unregister_stop_hook(detergent_stop_hook, NULL);
...
static esp_err_t detergent_do_cleanup(bool off_already_retried)
{
    esp_err_t first_err = s_rt.task_exit_result;
    if (first_err != ESP_OK && !off_already_retried) {
        esp_err_t r = apply_pump_off();
    ...
    if (s_rt.hook_registered) {
        esp_err_t h = safety_manager_unregister_stop_hook(detergent_stop_hook, NULL);
        if (h != ESP_OK) {
...
esp_err_t detergent_service_run_async(const detergent_request_t *request)
{
    ...
    if (request->duration_ms == 0 || request->duration_ms > s_rt.cfg.max_single_ms) return ESP_ERR_INVALID_ARG;
    xSemaphoreTake(s_lc_mtx, portMAX_DELAY);
    if (s_rt.lifecycle != LC_RUNNING || !s_rt.cmd_queue) { xSemaphoreGive(s_lc_mtx); return ESP_ERR_INVALID_STATE; }
    bool exp = false;
    ...
    detergent_cmd_t cmd = { .type = CMD_RUN, .request = *request };
...
esp_err_t detergent_service_cancel(machine_request_id_t request_id)
{
    ...
    xSemaphoreTake(s_lc_mtx, portMAX_DELAY);
    if (s_rt.lifecycle != LC_RUNNING || !s_rt.cmd_queue) { xSemaphoreGive(s_lc_mtx); return ESP_ERR_INVALID_STATE; }
    detergent_cmd_t cmd = { .type = CMD_CANCEL, .request.request_id = request_id };
...
esp_err_t detergent_service_emergency_stop(void)
{
    ...
    if (s_rt.cmd_queue && s_rt.lifecycle == LC_RUNNING) {
        detergent_cmd_t cmd = { .type = CMD_EMERGENCY };
...
/* Explicit mapping between internal detergent_fsm_state_t (IDLE=0) and the
 * public detergent_state_t (IDLE=1, ...) lives in detergent_fsm.c
 * (detergent_fsm_state_to_public, declared in detergent_fsm.h).  Same defect
 * class as the UV enum offset; relying on equal numeric values is forbidden. */
...
esp_err_t detergent_service_get_snapshot(detergent_snapshot_t *out)
{
    ...
    /* 快照必须确定性填充全部字段（HW-FIX-2）：terminal_pending 之前未赋值，
     * 是未初始化栈垃圾导致板测门禁 SERVICE_BUSY 误判。 */
    out->state = detergent_fsm_state_to_public(s_rt.fsm.state);
    out->request_id = s_rt.fsm.request_id;
    out->requested_duration_ms = s_rt.fsm.duration_ms;
    out->output_state = detergent_fsm_output_to_public(s_rt.fsm.output_state);
    out->terminal = s_rt.fsm.terminal;
    out->terminal_pending = detergent_fsm_terminal_pending(&s_rt.fsm);
    out->elapsed_ms = 0U;
    if (s_rt.fsm.state == DETERGENT_FSM_RUNNING) {
        uint32_t now_ms = (uint32_t)(xTaskGetTickCount() * portTICK_PERIOD_MS);
...
uint32_t detergent_service_test_get_mailbox_consumed(void) { return s_rt.emerg_mailbox_consumed; }
...
esp_err_t detergent_service_abort_cleanup(void)
{
    ...
    if (s_rt.hook_registered) { safety_manager_unregister_stop_hook(detergent_stop_hook, NULL); s_rt.hook_registered = false; }
    if (s_rt.cmd_queue) { vQueueDelete(s_rt.cmd_queue); s_rt.cmd_queue = NULL; }
    if (s_rt.snapshot_mtx) { vSemaphoreDelete(s_rt.snapshot_mtx); s_rt.snapshot_mtx = NULL; }
    memset(&s_rt, 0, sizeof(detergent_ctx_t)); s_rt.lifecycle = LC_UNINITIALIZED;

    atomic_store(&s_rt.reserved_request_id, MACHINE_REQUEST_ID_INVALID);
    xSemaphoreGive(s_lc_mtx); return ESP_OK;
}
esp_err_t detergent_service_test_reset(void) { return detergent_service_abort_cleanup(); }
bool detergent_service_test_resources_alive(void) { return s_rt.cmd_queue != NULL && s_rt.snapshot_mtx != NULL; }
#endif
